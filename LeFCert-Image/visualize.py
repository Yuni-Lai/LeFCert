...
def merged_certified_curve(df_combine, output_file, options):
    ...
    fixed_order = ['LeFCert-LD','LeFCert-L','LeFCert', 'FCert', 'DPA', 'KNN']
...
def compare_various_methods(options):
    ...
    for model_name in ['LeFCert-LD', 'LeFCert-L', 'LeFCert', 'FCert', 'KNN', 'DPA']:
        try:
            if model_name in ['LeFCert-L', 'LeFCert-LD']:
                CERTIFIY_TYPE = f"{options.certify_type}_sigma{options.sigma}_r{options.r}_ld{options.Lambda}"
            elif model_name == 'LeFCert':
                CERTIFIY_TYPE = f"{options.certify_type}_ld25"
            else:
    ...
    if dfs:
        output_file_path = f'./output/{options.dataset_type}_{options.model_type}/merged_certaccuracy_C{options.C}_K{options.K}_M{options.M}_sig{options.sigma}_r{options.r}.pdf'
        df_combine = pd.concat(dfs, ignore_index=True)
        merged_certified_curve(df_combine, output_file_path, options)
    part_of_df = df_combine[df_combine.loc[:, "poisoning size"].isin([0, 1, 3, 5, 7, 9])]
    print(f'Dataset:{options.dataset_type}, C{options.C}_K{options.K}_M{options.M}_sig{options.sigma}_r{options.r}')
...
def visualize_parameters(df_combine, parameter_type, parameter_values, options):
    ...
    file_path = options.file_path + f'{options.dataset_type}_{options.model_type}'
    plt.savefig(f"{file_path}/certified_curves_{parameter_type}.pdf", dpi=300, bbox_inches='tight')
    plt.show()
    print(f"Saved certified curves for {parameter_type} to {file_path}/certified_curves_{parameter_type}.pdf")

def compare_various_parameters(options, parameter_type):
    file_path = options.file_path + f'{options.dataset_type}_{options.model_type}/{options.certify_model}/C{options.C}_K{options.K}_M{options.M}'
    ...
    for value in parameter_values:
        ...
        temp_file_path = f'{file_path}/certify_result_{CERTIFIY_TYPE}.pkl'
...
def visualize_parameters2(df_combine, parameter_type, parameter_values, options):
    """
    Visualize certified accuracy curves for various parameter values.
    ...
    plt.figure(constrained_layout=True, figsize=(10, 6))
    if parameter_type== 'C':
        plt.title(fr"{options.dataset_type}, $(K,M$={options.K},{options.M})")
        palette = "coolwarm"
    else:
        plt.title(fr"{options.dataset_type}, $(C,M$={options.C},(K-1)/2)")
        palette="flare"
    df_combine = df_combine.rename(columns={'certify_model': 'Model'})
    ...
    sns.lineplot(
        ...
        style='Model',
        hue_order=parameter_values,
        palette=palette,#"coolwarm"， # "viridis"，"magma"
        data=df_combine,
        linewidth=3,
        dashes={"FCert": (3, 5),  "LeFCert": (None,None)}
    )
    if parameter_type == 'K':
        plt.xlim(0,18)
    ...
    plt.xlabel('Poisoning Size (T)')
    plt.ylabel('Certified Accuracy')
    plt.legend(loc='upper right', fancybox=True, ncol=1,framealpha=0.5)
    file_path = options.file_path + f'{options.dataset_type}_{options.model_type}'
    plt.savefig(f"{file_path}/certified_curves_{parameter_type}.pdf", dpi=300, bbox_inches='tight')
    plt.show()
    print(f"Saved certified curves for {parameter_type} to {file_path}/certified_curves_{parameter_type}.pdf")

def compare_various_parameters2(options, parameter_type):
    # Define the range of values for the parameter
    if parameter_type == 'C':
        parameter_values = [5,6,7,8]
    elif parameter_type == 'K':
        parameter_values = [10,15,20,25]
    else:
        raise ValueError(f"Unsupported parameter type: {parameter_type}")

    dfs = []
    for certify_model in ['LeFCert', 'FCert']:
        options.certify_model= certify_model
        for value in parameter_values:
            setattr(options, parameter_type, value)  # Update the parameter in options
            options.M = int((options.K - 1) / 2)  # default
            print(f"Loading results for {parameter_type}={value}")
            new_path = f'./output/{options.dataset_type}_{options.model_type}/{options.certify_model}/C{options.C}_K{options.K}_M{options.M}'
            # Construct file path
            if options.certify_model in ['LeFCert-L', 'LeFCert-LD']:
                CERTIFIY_TYPE = f"{options.certify_type}_sigma{options.sigma}_r{options.r}_ld{options.Lambda}"
            elif options.certify_model == 'LeFCert':
            ...
            file_path = f'{new_path}/certify_result_{CERTIFIY_TYPE}.pkl'
    ...
    if dfs:
        ...
        print("No results found for the specified parameter values.")
    visualize_parameters2(df_combine, parameter_type, parameter_values, options)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_type',type=str,help='',default="cubirds200",choices=['cifarfs','cubirds200','tiered_imagenet'])
    ...
    # compare_various_parameters(options, 'Lambda') #for cifar and iamgenet

    # compare_various_parameters2(options, 'C')# for cubird and imagenet

    # compare_various_parameters2(options, 'K')  # for cubird and imagenet
