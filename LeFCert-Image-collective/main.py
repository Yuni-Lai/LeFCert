...
import clip
from FCert import FCert, LeFCert,LeFCertL, LeFCertC
...
parser.add_argument('--certify_model', type=str, choices=['KNN','DPA','FCert','LeFCert','LeFCert-L','LeFCert-LD','LeFCert-new'], default="LeFCert-C")
parser.add_argument('--metric_type',type=str,default="cosine",choices=['cosine','l2'])
parser.add_argument( '--M',type=int,help='number of trimmed samples (K prime),default M = (K-1)/2')
...
options.device = 'cuda:0' if torch.cuda.is_available() and options.cuda else 'cpu'
...
with torch.no_grad():
    ...
    if options.certify_model == "FCert":
    ...
    elif options.certify_model == 'LeFCert-C':
        acc = LeFCertC(opt=options, test_dataloader=test_dataloader,
                     model=model, clip_k=M, T=Max_T, certify_type=CERTIFIY_TYPE, class_map=class_map)
    elif options.certify_model == 'LeFCert-L':
        acc = LeFCertL(opt=options,test_dataloader=test_dataloader,
                model=smoothed_classifier,clip_k=M,T=10,certify_type = CERTIFIY_TYPE,class_map=class_map)
        ...
        acc = certify_knn(opt=options, test_dataloader=test_dataloader,
                model=model, n=10, T=10, certify_type=CERTIFIY_TYPE )
    elif options.certify_model == "DPA":
        acc = certify_dpa( opt=options, test_dataloader=test_dataloader,
                model=model, n_partitions=K, T=10,certify_type = CERTIFIY_TYPE)
    end_time = time.time()
    print(f"Time taken for certification: {end_time - start_time:.2f} seconds")
...
elif options.certify_model in ['LeFCert', 'LeFCert-C']:
    result_dict['Lambda'] = options.Lambda
