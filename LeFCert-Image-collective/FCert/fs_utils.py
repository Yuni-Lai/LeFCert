...
def cosine_similarity(x,y):
    ...
    B = y
    # Calculate the cosine similarity for each pair of vectors
    similarity = 1+F.cosine_similarity(A.unsqueeze(1), B, dim=-1)
    return similarity
def l2_dist(x,y):
...
def get_f_f_dict(opt, input, target, n_support):
    ...
    if opt.metric_type =="cosine":
        ...
        dicts = l2_dist(query_samples, support_samples)
...
def get_f_t_dict(opt, input,text_features, target, n_support):
    ...
    if opt.metric_type =="cosine":
        ...
        dicts = l2_dist(text_features.to('cpu').type(torch.float32), support_samples)

    query_samples = input.to('cpu')[query_idxs]
    if opt.metric_type =="cosine":
        test_dicts=cosine_dist(query_samples,text_features.to('cpu'))
    elif opt.metric_type =="l2":
        test_dicts = l2_dist(query_samples,text_features.to('cpu').type(torch.float32))
    ...
    with torch.no_grad():
        if opt.model_type == "CLIP":
            if opt.certify_model == 'LeFCert-LD':
                model_output = model.encode_image_dif(x).float()
