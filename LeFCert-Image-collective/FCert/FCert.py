...
def box_bound(t, K, M, ff_sorted_dist,ft_sorted_dist,test_dicts,Lambda,num_query):
    if t<=K-M:
        ...
        ff_clipped_sum_upper = torch.full_like(ff_sorted_dist[:, :, 0], 2*K)  # upper bound is 2 *K
        ff_clipped_sum_lower = torch.full_like(ff_sorted_dist[:, :, 0], 0)  # lower bound is 0
        ft_clipped_mean_upper = torch.full_like(ft_sorted_dist[:, :, 0], 2)  # upper bound is 2
        ft_clipped_mean_lower = torch.full_like(ft_sorted_dist[:, :, 0], 0)  # lower bound is 0



    # Reshape ft_clipped_mean_upper and ft_clipped_mean_lower to match batch_size * num_classes
    ft_clipped_mean_lower = ft_clipped_mean_lower.repeat_interleave(num_query, dim=0)
    ft_clipped_mean_upper = ft_clipped_mean_upper.repeat_interleave(num_query, dim=0)
...
def normal_bound(t, K, M, ff_sorted_dist, ft_sorted_dist, test_dicts,Lambda,num_query):
...
def normal_bound_merge(t, K, M, sorted_dist,Lambda,num_query):
...
def LeFCert(opt, test_dataloader, model,class_map, clip_k=4, T=10, certify_type="ind"):
    ...
    f = open(f'{opt.file_path}/time_T.pkl', 'wb')
    pickle.dump(df, f)
    f.close()
    avg_acc = np.mean(avg_acc, axis=0)
    return avg_acc
...
def LeFCertNew(opt, test_dataloader, model,class_map, clip_k=4, T=10, certify_type="ind"):
    ...
    for batch_id, batch in enumerate(test_dataloader):
        ...
        ft_dicts, test_dicts = get_f_t_dict(opt, model_output, text_features, target=y, n_support=opt.K)
        ft_dicts = torch.reshape(ft_dicts, (ft_dicts.size(0), opt.C, opt.K))
        merged_dist = ff_dists + (Lambda / K) * test_dicts * ft_dicts
        ...
        if certify_type == "group":
            ...
            all_clipped_mean_lower = []
            for t in range(min(clip_k+1,T + 1)):
                clipped_mean_upper, clipped_mean_lower = normal_bound_merge(t, K, clip_k,merged_sorted_dist,
                                                                      opt.Lambda,opt.num_query)
                # else:
                #     clipped_mean_upper, clipped_mean_lower = box_bound(t, K, clip_k, ff_sorted_dist,
                #                                                        ft_sorted_dist, test_dicts, opt.Lambda)
...
# Generate all possible allocations of T budget across C classes
def generate_allocations(T, C):
    # Generate combinations with replacement for T items in C classes
    allocations = itertools.combinations_with_replacement(range(C), T)
    # Convert combinations to counts per class
    allocation_counts = []
    for allocation in allocations:
        counts = [allocation.count(i) for i in range(C)]
        allocation_counts.append(counts)
    return allocation_counts

def LeFCertC(opt, test_dataloader, model,class_map, clip_k=4, T=10, certify_type="ind"):
    device = opt.device
    model = model.to(device)
    num_batches = test_dataloader.__len__()
    avg_acc = np.zeros((num_batches, T))
    ...
    for batch_id, batch in enumerate(test_dataloader):
        ...
        if certify_type == "group":
            all_clipped_mean_upper = []
            all_clipped_mean_lower = []
            for t in range(T + 1):#min(clip_k+1,T + 1)):
                if t<= clip_k:
                    clipped_mean_upper, clipped_mean_lower = normal_bound(t, K, clip_k, ff_sorted_dist,
                                                                      ft_sorted_dist, test_dicts,
                                                                      opt.Lambda,opt.num_query)
                    ...
                    clipped_mean_upper, clipped_mean_lower = box_bound(t, K, clip_k, ff_sorted_dist,
                                                                       ft_sorted_dist, test_dicts, opt.Lambda,opt.num_query)
                all_clipped_mean_upper.append(clipped_mean_upper)
                all_clipped_mean_lower.append(clipped_mean_lower)
        ...
        for t in range(T):
            time_dict['T'].append(t)
            start_time = time.time()
            # Generate all possible allocations of T poisoning samples across C classes
            allocations = generate_allocations(t, C)
            # Traverse all allocations to find minimal certified accuracy
            min_correct_count = float('inf')
            for allocation in allocations:
                bounds = torch.zeros(batch_sizes, C)
                for i in range(batch_sizes):
                    for j in range(C):
                        if torch.flatten(target_inds)[i] == j:
                            bounds[i][j] = all_clipped_mean_upper[allocation[j]][i][j]
                        else:
                            bounds[i][j] = all_clipped_mean_lower[allocation[j]][i][j]

                preds = torch.argmin(bounds, dim=-1)
                correct_count = torch.sum(preds == torch.flatten(target_inds)).item()
                min_correct_count = min(min_correct_count, correct_count)
                if min_correct_count == 0:
                    break

                avg_acc[batch_id, t] = min_correct_count / batch_sizes
            end_time = time.time()
            time_dict['times'].append(end_time - start_time)
    df = pd.DataFrame(time_dict)
    f = open(f'{opt.file_path}/time_T.pkl', 'wb')
