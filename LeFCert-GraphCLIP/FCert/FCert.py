...
def FCert(opt, test_dataloader, model,clip_k=4,T=10,certify_type = "group",num_batches =20):
    device = opt.device
    ...
    for batch_id, batch in enumerate(test_dataloader):
        if batch_id == num_batches:
            break
        # get node embedding for this batch [total_nodes, feat_dim]
        node_features = get_feature(opt, model, batch, batch['node_id'].to(device))
        labels = batch.y.to(device)

        dist, target_inds = get_distances(opt, node_features, labels, opt.K)
...
def box_bound(t, K, M, ff_sorted_dist, ft_sorted_dist, test_dicts, Lambda):
    ...
    ff_clipped_sum_lower = torch.sum(ff_clipped_dist_lower, dim=-1) + 0  # min=0
    ...
    ft_clipped_mean_lower = torch.sum(ft_clipped_dist_lower, dim=-1) / (K - 2 * M)  # min=0
...
def MFCert(opt, test_dataloader, model,class_map, clip_k=4, T=10, certify_type="ind", num_batches=20):
    device = opt.device
    model = model.to(device)
    avg_acc = np.zeros((num_batches, T))
    ...
    for batch_id, batch in enumerate(test_dataloader):
        if batch_id == num_batches:
            break
        # get the label text
        classes = torch.unique(batch['label'])
        label_text = [f"this paper has a topic on {class_map[class_id.item()]}," for class_id in classes]
        label_text = [ti + desc for ti, desc in zip(label_text, opt.c_descs)]
        label_text_tokens = tokenizer(label_text, truncation=True, padding=True, return_tensors="pt", max_length=512).to(device)
        text_features = model.encode_text(label_text_tokens["input_ids"], label_text_tokens['token_type_ids'], label_text_tokens["attention_mask"])# t_k
        graph_features = get_feature(opt, model, batch, batch['node_id'].to(device))
        # graph_features = graph_features / graph_features.norm(dim=1, keepdim=True)
        # text_features = text_features / text_features.norm(dim=1, keepdim=True)

        ff_dicts, target_inds = get_f_f_dict(opt, graph_features, target=batch['label'].to(device), n_support=opt.K)
        ff_dicts = torch.reshape(ff_dicts, (ff_dicts.size(0), opt.C, opt.K))
        ft_dicts, test_dicts = get_f_t_dict(opt, graph_features, text_features, target=batch['label'].to(device), n_support=opt.K)
...
def MLFCert(opt, test_dataloader, model, class_map, clip_k=4, T=10, certify_type="group", num_batches=20):
    device = opt.device
    model = model.to(device)
    avg_acc = np.zeros((num_batches, T))
    tokenizer = AutoTokenizer.from_pretrained('sentence-transformers/all-MiniLM-L6-v2')
...
def MFCert(opt, test_dataloader, model, class_map, clip_k=4, T=1, certify_type="ind", num_epochs=1, num_batches=50):
    # certification fails if T>k'
    if T > clip_k:
        if opt.box_constraint == False or T > opt.K - clip_k - 1:
            return 0
    ...
    for epoch in range(num_epochs):
        for batch in test_dataloader:
            ...
            label_text_tokens = tokenizer(label_text, truncation=True, padding=True, return_tensors="pt", max_length=512).to(device)
            text_features = model.encode_text(label_text_tokens["input_ids"], label_text_tokens['token_type_ids'], label_text_tokens["attention_mask"])# t_k
            graph_features = get_feature(opt, model, batch['node_id'].to(device))
            # graph_features = graph_features / graph_features.norm(dim=1, keepdim=True)
            # text_features = text_features / text_features.norm(dim=1, keepdim=True)
            ...
            if certify_type == "group":
                ...
                for t in range(T + 1):
                    if t <= clip_k:
                        clipped_mean_upper, clipped_mean_lower = normal_bound(t, K, clip_k, ff_sorted_dist,
                                                                              ft_sorted_dist, test_dicts, opt.Lambda)
                    elif opt.box_constraint:
                        clipped_mean_upper, clipped_mean_lower = box_bound(t, K, clip_k, ff_sorted_dist, ft_sorted_dist,
                                                                           test_dicts, opt.Lambda)
                    all_clipped_mean_upper.append(clipped_mean_upper)
                    all_clipped_mean_lower.append(clipped_mean_lower)
